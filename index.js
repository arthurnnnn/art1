...
async function starts() {
	...
	Arthur.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				try {
					ppimg = awat client.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
				...
				num = anu.participants[0]
				try {
					ppimg = awat client.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
	...
	Arthur.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD ARTHUR BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de arthur. Registre-se para fazer amizade com o bot arthur por meio, \n\nCommand : ${prefixbotbot}daftar nama|idade\nExemplo : ${prefixbotbot}daftar Arthur|17\n\n──「 arthur BOT 」──`,
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: ' [❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
				}
			}

			const botNumber = Arthur.user.jid
			const ownerNumber = ["557592039670@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"557592039670@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["557592039670@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["557592039670@s.whatsapp.net"]//ubah nomor lo
			const premium = ["557592039670@s.whatsapp.net","559784130980@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net","12267740582@s.whatsapp.net"]
			const sGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? awat Arthur.groupMetadata(from) : ''
			...
			const reply = (teks) => {
				Arthur.sendMess(from, teks, text, {quoted:mek})
			}
			const sendImage = (teks) => {
		    Arthur.sendMess(from, teks, image, {quoted:mek})
		    }
			const sendMess = (hehe, teks) => {
				Arthur.sendMess(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? Arthur.sendMess(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : Arthur.sendMess(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
              //function leveling
		...
		Arthur.updatePresence(from, Presence.composing)

		reply(`tchau racista ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		setTimeout( () => {
			Arthur.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		{, 5000)
		setTimeout( () => {
			Arthur.updatePresence(from, Presence.composing)
